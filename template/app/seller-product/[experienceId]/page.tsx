...
export default async function Page({ params: { experienceId } }: PageProps<"experienceId">) {
	...
	if (
		...
		return (
			<div>
				You are not authorized to view this page. You must be an admin of the company. (Make
				sure you are developing in the whop.com iframe if testing)
			</div>
		);
	}
	...
	return (
		<div>
			...
			<p>Attached to products: {experience.data.product_ids.join(", ") || "None yet."}</p>
		</div>
	);
}
